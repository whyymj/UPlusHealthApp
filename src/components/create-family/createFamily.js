...
export default {
  ...
  methods: {
    ...
    openPicker (index) {
      ...
      switch (index) {
        ...
        case 3:
          ...
          this.dateSlots[0].defaultIndex = this.relationshipValue ? this.relationList.findIndex(_ => { return _.dict_name === this.relationshipValue }) : 0
          this.valuePicker = this.relationshipValue ? this.relationObj : this.relationList[0]
          break
      }
    ...
    async createProfile () {
      // 创建健康档案
      let data = {
        relation: this.relationObj.dict_id,
      ...
      console.log(data)
      if (this.heightValue === '' || this.weightValue === '' || this.birthDayValue === '') {
        Toast({
          message: '信息不全'
        })
      } else {
        try {
          ...
          if (result.data.code === 'C0000') {
            window._member_id = result.data.data.member_id
            this.popupVisible = true
            this.icon = 'fa fa-check-circle-o'
            this.title = `创建人物-${result.data.data.relation_name} 成功`
