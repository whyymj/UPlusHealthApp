<template>
  <div class="page suger-page">
    <!-- <h-header title="血糖"></h-header> -->
        <mycollapse2 animateT='500'>
        <div>
          <myDatePicker @checkDateData='checkDateData' flag='4'></myDatePicker>
        </div>
      </mycollapse2>
    <div class="container has-footer">
      ...
      <div :class="['calendar-below', {'open' : calendarOpen}]" id="calendarBelow">
        <div ref="noData" class="sugar-record no-record card-container">
          <div>
            <span>暂无数据，点击按钮去添加吧。</span>
            <button class="default-btn record-btn" @click="goManualEntry('entry-sugar')">去添加</button>
          </div>
        </div>
        <div class="sugar-record-list-c">
          <div v-for="(item, index) in sugarRecordData" :key="index" :class="['sugar-record-list', 'card-container', {'sugar-toggle': index + 1 === sugarIndex}]">
            <mt-cell-swipe :right="[
                  {
              ...
              <div @click="openHealthTips(index + 1)" :class="['record-item']">
                ...
                <div class="record-heart text-right">
                  ...
                  <div class="record-heart-time">
                    <span>测量时间</span><span>{{item.bloodSugar.create_date.split(' ')[1].slice(0, -3)}}</span>
                  </div>
                  <div class="record-heart-time">
                    <span>{{item.bloodSugar.time_point}}</span>
                  </div>
                </div>
          ...
          <div ref="allData" class="sugar-record-list card-container" style="display: none" @click="fnGetAllData()">
            <p style="text-align: center; padding: .5rem; color: #b5b5b5;">
              查看全部数据
            </p>
          </div>
        </div>
        <div class="card-container sugar-chart">
          <h2>趋势图</h2>
          ...
          <div class="chart">
            <chart ref="sugar" :options="sugarChartsOption" :autoResize="true">
            </chart>
            <!--no data-->
            <div ref="noSugar" class="no-result-chart" style="display: none">
              <img src="../../assets/no-result-chart.png" alt="">
              <div class="title">记录血糖后将在此看到变化曲线</div>
            </div>
            <!---->
          </div>
        </div>
